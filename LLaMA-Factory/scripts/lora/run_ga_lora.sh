...
for name in "${names[@]}"; do
  ...
  PYTHONPATH=./ WANDB_DISABLED=true python src/train_bash.py --stage ga \
    --model_name_or_path microsoft/Phi-3-mini-4k-instruct --do_train --save_model \
    --dataset ${id}_Positive --dataset_dir ./data --finetuning_type lora --lora_target q_proj,k_proj,v_proj,o_proj \
    --output_dir ./saves/unlearn_bench/People/${id}/ga_lora/phi3_mini_4k_instruct --overwrite_cache \
    ...
    --output_result_dir ./results/unlearn_bench/People/${id}/ga_lora/phi3_mini_4k_instruct \
    --fp16 --eval_dataset_dir ./data/unlearn_bench/People/ \
    --target ${id} 2>&1 | tee ./logs/unlearn_bench/People/ga_lora/phi3_mini_4k_instruct_${id}.log
done
